...
while read -r container; do
	...
	docker run --env-file /var/www/$SUBDOMAIN/.env --rm --volumes-from $container --network=docker_sqlnet --link sqldb.noflag.org.uk:sqldb.noflag.org.uk $IMAGE:latest sh -c 'wp --allow-root core update-db; w
p --allow-root plugin update --all'
