...
Where possible, containers should always be named after a DNS where they can be accessed, for example `sqldb.noflag.org.uk`. Even though SQLDB cannot be accessed from the internet, it in theory could be, and there might be reasons why you would need that. So it gets named after its DNS.
